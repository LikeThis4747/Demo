...
```
Engine\Build\BatchFiles\Build.bat MyProjectEditor Win64 Development ${workspaceFolder:MyProject}\MyProject.uproject -waitmutex
```

或使用 VS Code 任务：**MyProjectEditor Win64 Development Build**。
...
- `assetPath`（必填）：完整资产路径（如 `/Game/MyProject/Blueprints/BP_Foo`）
...
```
Engine\Build\BatchFiles\Build.bat MyProjectEditor Win64 Development ...
```
