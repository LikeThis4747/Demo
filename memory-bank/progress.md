...
- [x] 冲锤 Heavy 增加来源响应比例，默认保留 60% 首次接触共同平移，并在首次真实接触后结束 PhysicsBody 持续顶推；构建、装配回读与受击自动化通过，待用户 PIE 手感验收。
...
- Heavy 来源响应比例默认 1.0；当前只有冲锤资产使用 0.60。该参数不替代真实 Chaos 接触，也不修改摆锤和追猎者攻击的既有效果。
