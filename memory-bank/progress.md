...
- [x] 玩家/追猎者共享 HeavyImpact 真实 Chaos 受击与提前 Snapshot 起身恢复桥；旧可见物理姿势准备已删除，Demo 模块构建、双 MCP 资产回读及 5/5 自动化已通过。
- [ ] HeavyImpact 正躺/趴倒、当前物理 Snapshot 到 Montage 的画面连续性、墙边/墙角、完全堵塞/解除、二次接触与恢复移动的用户验收。
...
- HeavyImpact 提前 Snapshot 单一混合已完成代码、AnimBP、参数、构建和自动化；真实命中连续画面、墙边和恢复追逐仍待用户验收。
