...
- [x] 玩家/追猎者共享 HeavyImpact 真实 Chaos 受击与提前 Snapshot 起身恢复桥；用户已验收当前受击起身主效果。新增防夹保护：真实碰撞保留 0.15 秒后仅释放角色网格的 PhysicsBody 接触，恢复后同来源短期免疫；Demo 模块构建、官方 MCP 回读及 5/5 自动化已通过。
- [ ] HeavyImpact 防夹增量现场复测：摆锤夹住时仍能恢复、玩家同来源保护 0.75 秒、追猎者同来源保护 3.0 秒；并补测墙边/墙角、第二机关来源与恢复追逐。
- [ ] 解释并复现最新保存 PIE 日志中的无地面支撑硬超时；当前不把无上下文日志判为代码缺陷。
- [ ] 在具备运行时 Recast 的正式环境确认追猎者起身后恢复追逐。
...
- HeavyImpact 提前 Snapshot 单一混合的当前受击起身主效果已由用户验收；防夹增量已完成代码、DataAsset、构建、自动化与短 PIE 技术验证，仍待用户在摆锤夹持场景现场复测。
- 静态 Recast Actor、Blueprint UpToDate、资产非脏、自动化和日志均不能替代玩家实际行走、真实追逐与用户体验验收。
