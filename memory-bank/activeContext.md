# Active Context — Demo

## 当前已确认基线

- PCG Population 已完成机关优先、资源后置的分层放置并由用户验收；Normal Seed 12345 代表结果为机关 36/36、资源 13/13。
- HeavyImpact 已验收：真实 Chaos 位移、有限 Physics Control、Snapshot 起身、同来源保护；HeavyImpact 5/5。
- 追猎者近战斧击与预判跑跳 Heavy 攻击已经技术交付并由用户看到击飞效果；不要无依据调整其预测、命中或击飞参数。
- Light 当前验收：制导/磁力命中玩家为 Slow 0.40 秒、倍率 0.55、无动画、局部物理；磁力命中追猎者为 Stop 0.60 秒、三方向动画、局部物理；地刺命中玩家为 Stop 0.25 秒、三方向动画、无局部物理。异常前冲已修复；局部物理偏弱但用户接受当前版本。
- 新机关受击接入权威：`DOC/Outputs/Physics/CHARACTER_IMPACT_INTEGRATION_GUIDE.md`。Light/Heavy 接收核心不包含机关类型特判。

## 当前 P0

1. 在同一正式一局中证明动态 Recast 和真实追猎者多层追逐。
2. 用户验收追猎者跑跳/近战的预警、可躲性、落空恢复与动画观感。
3. 确认并灰盒化最小目标链与首批音效。
4. 完成首轮 Development 打包与整局回归。

## 受击架构边界

- 新机关 None/Slow/Stop：来源 DataAsset + 稳定事件 ID + 一次 `FStandingImpactRequest`；不得修改 CharacterImpact 核心或 Cast 具体角色。
- 新机关 Heavy：机关侧实现接触前预测并由真实动态刚体接触；不得在 Heavy 失败后对同一接触降级补 Light。
- 机关自己的碰撞、销毁、伤害、持续力和破碎不进入角色受击组件。
- 不恢复全身常驻受控物理、Light/Heavy 大一体化或跨对象磁力租约方案。

<!-- written by shiqiqiwang at 2026-08-13 13:40 UTC -->

## 2026-08-13 刺轮技术初版（待手感验收）

- Level0 已放置一个项目自有刺轮：按实例 Seed 从对应 1～3 格路线模板中选择镜像、方向和起始相位；开放路线往返，闭合路线循环。
- 玩家接触提交 Stop 并扣 20 点生命；机关自身使用离开再进入 + 1.0 秒局部门禁，追猎者映射 None。玩家 Stop 三方向动画已经补齐并获用户现场确认。
- 当前只验证手工两格实例，不接入 Population/PCG；跨格足迹、占用格合同和组合投放留到用户确认路线压力后再设计。

<!-- written by shiqiqiwang at 2026-08-13 14:07 UTC -->

- 刺轮展示调整：Level0 实例已按用户反馈移至冲锤测试区最前方的独立空房间 `(53500, 30000, 0)`，当前使用一格闭环路线以完整留在约 600×600 cm 室内；1～3 格模板能力不变，仍待用户手感验收。


## 2026-08-13 玩家 Stop 动画补齐

- `DA_PlayerStandingImpact` 已装配玩家 Front/Left/Right 三条重定向 Sequence；地刺玩家结果保持 Stop 0.25 秒并开启方向动画。
- 用户已现场确认当前地刺 Stop 动画可用；少量观感细节留待次日按实际反馈调整，不先修改受击核心。

<!-- written by shiqiqiwang at 2026-08-13 15:25 UTC -->

## 2026-08-13 PCG 固定 Seed 与软质量收敛（待用户验收）

- 公开 Seed 自动换种和关卡重载已移除；Version 字段删除，旧 V7 随机扰动融合为内部盐，Population 随机合同保持。
- 普通格/总量/覆盖/长直线/端点与出生距离均为软目标；WFC 最多三棵确定性树、首个硬合法候选保底、完美候选提前停止，无解时同层确定性连通兜底。
- 正式 Profile 每栋至少两个高厅且至少一个非顶层；高厅先于可选楼梯。三档各 300 Seed、每个重放两次的 1800 次整栋求解全部通过。
- 自动化不替代真实动态 Recast、全楼梯实走与地图观感；非法配置/代码不变量/导航故障仍显式失败，但不得换 Seed。
