...
- 发射器仅占挂载锚点一格，正前方不再作为内容硬冲突；合法方向与运行时净空检查保留。
- 正式权重 Easy/Normal/Hard 为 3/4/6；转角奖励 1.5，距离 2/3 格组合奖励 0.75/1.5，均为不拒绝候选的软评分。
- 摆锤、地刺、冲锤、刺轮从公开 Seed/类型/锚点派生相位，同组有限候选择优错开；玩家触发型发射器无周期相位。
- 技术验证通过，待用户验收出现量、组合可读性和错相节奏。
