...
- Light 当前验收：制导/磁力命中玩家为 Slow 0.40 秒、倍率 0.55、无动画、局部物理；磁力命中追猎者为 Stop 0.60 秒、三方向动画、局部物理；地刺命中玩家为 Stop 0.25 秒、三方向动画、无局部物理。异常前冲已修复；局部物理偏弱但用户接受当前版本。
- 新机关受击接入权威：`DOC/Outputs/Physics/CHARACTER_IMPACT_INTEGRATION_GUIDE.md`。Light/Heavy 接收核心不包含机关类型特判。

## 当前 P0

1. 在同一正式一局中证明动态 Recast 和真实追猎者多层追逐。
2. 用户验收追猎者跑跳/近战的预警、可躲性、落空恢复与动画观感。
3. 确认并灰盒化最小目标链与首批音效。
4. 完成首轮 Development 打包与整局回归。
...
- 玩家接触提交 Stop 并扣 20 点生命；机关自身使用离开再进入 + 1.0 秒局部门禁，追猎者映射 None。玩家 Stop 三方向动画已经补齐并获用户现场确认。
- 当前只验证手工两格实例，不接入 Population/PCG；跨格足迹、占用格合同和组合投放留到用户确认路线压力后再设计。

<!-- written by shiqiqiwang at 2026-08-13 14:07 UTC -->

- 刺轮展示调整：Level0 实例已按用户反馈移至冲锤测试区最前方的独立空房间 `(53500, 30000, 0)`，当前使用一格闭环路线以完整留在约 600×600 cm 室内；1～3 格模板能力不变，仍待用户手感验收。


## 2026-08-13 玩家 Stop 动画补齐

- `DA_PlayerStandingImpact` 已装配玩家 Front/Left/Right 三条重定向 Sequence；地刺玩家结果保持 Stop 0.25 秒并开启方向动画。
- 用户已现场确认当前地刺 Stop 动画可用；少量观感细节留待次日按实际反馈调整，不先修改受击核心。
