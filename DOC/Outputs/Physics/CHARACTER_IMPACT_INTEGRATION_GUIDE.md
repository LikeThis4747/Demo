...
> 状态：当前权威接入说明。最后核对：2026-08-13；玩家 Stop 三方向动画已补齐并在地刺路径获得用户现场确认。
...
| 地刺 | Stop 0.25s、三方向动画、无局部物理 | Slow 0.60s、速度 0.45、无动画/局部物理 |
...
- 地刺命中玩家：`Stop 0.25s / 三方向动画 / 无局部物理`。
- 局部物理观感仍偏弱，但当前可用；不再为单个机关扩写新的物理架构。

玩家 Stop 表现资产已经补齐：

- `DA_PlayerStandingImpact` 已装配 Front、Left、Right 三条玩家 Skeleton 动画副本。
- Stop 来源只需在自身 Profile 中设置 `bPlayReactionAnimation=true`，不需要修改 CharacterImpact 核心或 AnimBP。
- 当前地刺已按该方式启用玩家 Stop 动画；玩家 Slow 仍不播放全身动画。
- 后续新增 Stop 机关可直接复用这三方向动画。若某个机关需要不同动画，只替换角色 Tuning 或扩展明确的表现需求，不在机关 C++ 中直接播放角色动画。
