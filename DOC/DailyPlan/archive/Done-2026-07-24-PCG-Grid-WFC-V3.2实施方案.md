...
> 状态：已被后续带全局约束与回溯的 V4/V5 实现取代并归档，仅作历史留痕，AI 无需主动阅读。

> 2026-07-25 状态更正：本方案已经实施，但 PIE 玩家视角验收发现“固定中央长直路、路线过少、无运行时室内灯”，路线部分未通过验收。不得继续按本文的“固定中央主干 + Optional Envelope + 无回溯”作为下一版实现依据。修正讨论稿见 `claude/docs/2026-07-25-pcg-route-generation-revision-draft.md`；新方案经用户确认并展示拟实现代码前，不修改生成源码。
>
> 历史状态：V3.2 已获授权并完成实施；本文件保留当时的实施依据。2026-07-25 玩家验收否决的路线部分以最新修正讨论稿为准。
>
