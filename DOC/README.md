...
- `Design/CHALLENGE_DESIGN.md`：挑战性专题——追猎者改为**持续追击**（不再用丢失半径放弃，玩家收益改为体现在距离与时间）、机关摆放从全局均匀间距改为**成组遭遇 + 必经窄点**、机关候选评估（推荐刺轮与压力板触发，砍掉激光，不采纳多追猎者）、参数化要求与止损顺序。**本案修订 `ENEMY_DESIGN.md` 3.1 的感知规则。**
