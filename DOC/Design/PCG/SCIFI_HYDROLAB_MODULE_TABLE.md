...
> 状态：结构测量、Level0 代表组合和正式多层 Runtime Presentation 已落盘；构建、23 项 Automation 与代表性正式流程 PIE 已通过。当前等待玩家/追猎者真实跨层通行、视觉验收和 900 Seed 分布测试。
...
## 12. 正式多层 PCG 配方与运行时证据

### 12.1 已录入配方

| 完整结构 | Level0 样板基准 | 逻辑接口 | 公共件 | 开口组合件 | 隐藏坡面 | 合计 |
|---|---|---|---:|---:|---:|---:|
| 双层楼梯 `TwoFloorStair_Level0_A_2x2` | `(42300,5700,0)` | 下层东口、上层北口 | 83 | 41 | 2 | 126 |
| 贯通三层的楼梯间 `ThreeFloorStairwell_Level0_T_2x2` | `(44100,8100,0)` | 一层北口、二层西/南口、三层西口 | 71 | 77 | 4 | 152 |
| 高天花板房间 `HighCeilingRoom_Level0_1x2` | `(42900,8100,0)` | 南北贯通 | 16 | 20 | 0 | 36 |

- 上表基准坐标只用于从 Level0 只读样板提取相对 Transform；运行时只读取项目 Presentation DataAsset，不依赖关卡 Actor 名或这些世界坐标。
- 双层楼梯和贯通三层的楼梯间使用完整固定配方。可见 `StairsB` 忽略 Pawn 且不影响导航；隐藏 Cube 坡面阻挡 Pawn、参与导航、游戏中不可见并关闭阴影/主通道/深度/光追/场景捕获。
- 高天花板房间同步保留上层净空；允许某层为零，整栋最低数量、目标数量权重、每层上限和间距由 Generation DataAsset 调整。
- 贯通三层的楼梯间最多一座，只是可选捷径，不能替代每对相邻楼层至少一座必需双层楼梯。

### 12.2 HISM 与正式流程证据

- 表现构建先按相同 Mesh、碰撞职责、导航职责、隐藏和阴影设置合并实例，再每组一次调用 HISM 批量提交；不为配方中的每个 Piece 创建独立组件。
- 代表性正式 PIE 的成功轮为 3 层、121 个普通格和 5 个完整结构，共 2536 个实例，最终合并为 15 个 HISM 组件。
- 该轮对 11 个导航代表点完成投射，并从追猎者点验证 10 条完整路径，访问 1272 个导航节点，查询约 `0.150ms`；随后成功生成 24 个玩法对象及玩家、追猎者和 Exit。
- 原始 Seed `12345` 因整栋共享 WFC 预算失败，正式跨关卡重试派生 Seed `2044445496` 后成功。这证明失败关闭和重试流程可运行，但也说明仍需 900 Seed 测试校准成功率与长尾。
- `/Game/Levels/Level0` 和 `/Game/Assets/SciFiHydroLab/**` 在本次运行时实现中保持只读；没有遗留一次性导航 Actor、测试关卡、调试探针或临时资产。
- 自动化和单次 PIE 不能代替玩家连续实走、追猎者真实跨层追逐与最终视觉验收；这些项目通过前，当前 PCG 任务保持 Active。
