...
- 删除 Settling 抢跑起身与 5/10 秒正常硬切。身体有可行走支撑并持续处于低能量后才进入 Downed，小幅求解噪声只衰减稳定进度。
- Downed 睡眠完成后立即尝试起身；正常安全站位每 0.20 秒重试，最多 3.0 秒。截止时先以受击前位置为种子进行 200 cm 有界安全搜索；仍无解则按已确认玩法取舍回到受击前记录的角色变换、跳过起身动画并恢复控制，避免永久 Downed。
- Snapshot 到起身动画的淡入改为 0.30 秒。

## 调参与资产回读

- 玩家与追猎者 Heavy：`MaximumPreparationSeconds=0.18`、`RecoveryRetrySeconds=0.20`、`MaximumRecoveryBlockedSeconds=3.0`、`RecoverySnapshotBlendSeconds=0.30`。
...
- `DemoEditor Win64 Development` 完整构建成功，9 个 Action，正式链接通过。
...
- 人为阻塞起身超过 3 秒，确认正常搜索或最终位置回退后恢复操作，不永久倒地。
