...
- 每个关键状态必须由一个明确的模块或对象统一管理；其他系统只读取该状态或响应其变化。
...
- 编码方案必须列出每个新增/修改文件的完整路径、职责、关键状态管理职责、依赖及并行冲突；用户确认后才能创建。
...
- 每个新增或实质修改的 C++ 文件必须在文件顶部说明职责、边界与关键状态的唯一管理职责，便于快速判断代码归属。
...
编码前输出简短说明：核心循环、类/组件职责、关键状态的唯一管理者、C++/蓝图/数据边界、文件与资产清单、性能风险和验证方式。
