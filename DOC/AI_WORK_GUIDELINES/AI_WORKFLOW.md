...
3. **授权实现**：只有用户明确允许后才能落盘。每一步坚持最小修改，形成可独立验证、可回退的检查点，不夹带无关重构。
4. **分步验证**：每个检查点记录改动、验证结果和回退边界。需要用户操作蓝图或编辑器时，在操作前说明执行时机、前置条件、逐步操作和预期结果；不得用模糊指令代替步骤。UE 编辑器的组件、面板和属性名称优先使用“中文名称（English/C++ Name）”双语标注，例如“偏航（Yaw）”“目标臂长（Target Arm Length）”，方便中文版 UE 定位。
5. **联合验收**：代码与蓝图/编辑器操作完成后一起运行验收场景并检查边界情况。AI 可标记“待用户验收”，只有用户确认验收通过后才能标记任务完成并归档。
...
`讨论方案（claude/） → 用户确认 → DailyPlan → 授权实现与分步验证 → 夜间 DailyReport`
