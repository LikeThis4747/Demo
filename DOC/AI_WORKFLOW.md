...
1. **讨论方案**：说明目标、非目标、职责与状态 Owner、C++/蓝图/数据边界、文件范围、并行拆分或依赖、风险和验收方式。
2. **确定方案**：与用户确认方案；在对话中贴出本步拟新增或修改的代码，并标明尚未写入项目。
3. **授权实现**：只有用户明确允许后才能落盘。每一步坚持最小修改，形成可独立验证、可回退的检查点，不夹带无关重构。
4. **分步验证**：每个检查点记录改动、验证结果和回退边界。需要用户操作蓝图或编辑器时，在操作前说明执行时机、前置条件、逐步操作和预期结果；不得用模糊指令代替步骤。
...
6. 对白天已有改动执行一次 Git commit 和 push；不得为了让提交成功而修改项目文件。推送目标为**内部工蜂**，详见 `DOC/GIT_INTERNAL.md`。执行前须确认 `git remote get-url origin` 指向 `git@git.woa.com:shiqiqiwang/Demo.git`，**禁止推送到 GitHub**。
