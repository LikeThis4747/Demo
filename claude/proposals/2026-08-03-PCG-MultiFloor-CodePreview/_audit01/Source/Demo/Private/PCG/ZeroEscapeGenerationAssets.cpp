...
bool UZeroEscapeLevelGenerationProfile::IsConfigured(FString& OutError) const
{
	...
	for (const FZeroEscapeDifficultyDefinition& Definition : Difficulties)
	{
		...
		for (const FZeroEscapeFloorCountOption& Option : Definition.FloorCountOptions)
		{
			...
			if (Option.MinTotalWalkableCellCountPerFloor <= 0
				|| Option.MaxTotalWalkableCellCountPerFloor
					< Option.MinTotalWalkableCellCountPerFloor
				|| Option.MaxTotalWalkableCellCountPerFloor > GridCellCount
			...
			{
				return Fail(TEXT("FloorCountOption 的总量、每层普通内容或额外楼梯上限非法。"));
			}
