...
namespace ZeroEscape::LevelGeneration
{
	namespace MultiFloorLayoutPrivate
	{
			...
			const int32 LowerFloor)
		{
			...
			const int32 TotalWeight = Weights.ZeroAdditionalWeight
			...
			FZeroEscapeGenerationReport& OutReport)
		{
			...
			for (int32 LowerFloor = 0; LowerFloor + 1 < State.FloorCount; ++LowerFloor)
			{
				const int32 Target = DrawAdditionalTwoFloorStairCount(Input, LowerFloor);
				for (int32 AdditionalIndex = 0;
					AdditionalIndex < Target
						&& AdditionalPlaced < FloorOption.MaxAdditionalTwoFloorStairCount;
					++AdditionalIndex)
				{
			...
			const FPlacementState& State)
		{
			...
			for (const FZeroEscapeGeneratedStructure& Structure : State.Structures)
			{
				...
				Minimum = FMath::Min(
					Minimum,
					DistanceSquared2D(
						Candidate.BaseCoordinate,
						Structure.BaseCoordinate));
			}
			return Minimum;
		}
	...
	bool FMultiFloorLayoutPlanner::ResolveStructureForTesting(
		const FZeroEscapeStructureDefinition& Definition,
		const FIntPoint GridSize,
		...
		FString& OutError)
	{
		...
		const int64 CellCount = static_cast<int64>(FloorCount)
		...
		if (FloorCount <= 0
			...
			|| QuarterTurnCount > 3)
		{
			OutError = TEXT("测试结构解析的网格、楼层或旋转非法。");
			return false;
		}
		State.Cells.SetNum(static_cast<int32>(CellCount));

		FStructureCandidate Candidate;
		Candidate.DefinitionId = Definition.DefinitionId;
		Candidate.Kind = Definition.Kind;
		Candidate.BaseCoordinate = BaseCoordinate;
		Candidate.QuarterTurnCount = QuarterTurnCount;
		if (!BuildCandidateCells(
				Definition,
				State,
				BaseCoordinate,
				...
				Candidate))
		{
			OutError = TEXT("测试结构的单元格、连接或落脚点无法解析。");
			return false;
		}

		const FZeroEscapeStructureOpeningSetDefinition* OpeningSet =
			Definition.AllowedOpeningSets.FindByPredicate(
				[OpeningSetId](const FZeroEscapeStructureOpeningSetDefinition& Value)
				{
					return Value.SetId == OpeningSetId;
				});
		if (OpeningSet == nullptr)
		{
			OutError = TEXT("测试结构缺少指定的开口组。");
			return false;
		}
		TSet<FIntVector> Walkable;
		TSet<FIntVector> Footprint;
		for (const FIntVector Cell : Candidate.WalkableCells)
		{
			Walkable.Add(Cell);
			Footprint.Add(Cell);
		}
		for (const FIntVector Cell : Candidate.SolidCells)
		{
			Footprint.Add(Cell);
		}
		for (const FIntVector Cell : Candidate.ClearanceCells)
		{
			Footprint.Add(Cell);
		}
		if (!TryBuildOpeningsForSet(
				...
				*OpeningSet,
				...
				Candidate.Openings))
		{
			OutError = TEXT("测试结构的指定开口组无法在当前位置解析。");
			return false;
		}

		OutStructure.StableStructureId = 0;
		OutStructure.DefinitionId = Definition.DefinitionId;
		OutStructure.Kind = Definition.Kind;
		OutStructure.BaseCoordinate = BaseCoordinate;
		OutStructure.QuarterTurnCount = QuarterTurnCount;
		OutStructure.ActiveOpeningSetId = OpeningSetId;
		OutStructure.WalkableCells = MoveTemp(Candidate.WalkableCells);
		OutStructure.SolidCells = MoveTemp(Candidate.SolidCells);
		OutStructure.ClearanceCells = MoveTemp(Candidate.ClearanceCells);
		OutStructure.InternalConnections = MoveTemp(Candidate.InternalConnections);
		OutStructure.Openings = MoveTemp(Candidate.Openings);
		OutStructure.Landings = MoveTemp(Candidate.Landings);
		return true;
	}
#endif
}
