...
```text
git apply --check --recount claude/proposals/2026-08-03-PCG-MultiFloor-CodePreview/01-data-contract-core.patch
```
...
- 同一个 `EZeroEscapeStructureKind` 可以有多条定义。Planner 必须先按 Kind 筛选，再按 `DefinitionId` 字符串稳定排序后枚举候选；不得用 DataAsset 作者数组顺序参与确定性。
- 双层楼梯始终至少需要一条定义。
...
- 普通格必须按 `Z/Y/X` 严格排序；结构必须按 `StableStructureId` 严格递增。Planner 还需稳定排序结构内部数组，不能让 `TMap/TSet` 遍历顺序进入 Plan。
...
- Openings、Landings、AllowedOpeningSets 分别按稳定 ID 的字符串内容排序；每个 OpeningSet 内的 `OpenOpeningIds` 同样排序。
...
`FZeroEscapeGenerationReport.OperationId` 由 Runtime Generator 分配；纯数据测试保持 `0`。`FZeroEscapeGenerationMetrics.FloorWfcMetrics` 按楼层号保存每层二维 WFC 的观察、求解树、候选、传播、矛盾和回溯统计，整栋累计字段继续保留；这些搜索诊断只属于 Report，不进入 Plan 或规范 Hash。导航 Metrics 包含投射数、路径存在性检查数、`TestPathSync` 累计访问节点数和导航验证耗时。
...
它们覆盖：可调 GridSize、整栋总可走量与每层普通内容下限的不同语义、每层 WFC 报告默认值与楼层号、同 Kind 多定义、条件式结构定义要求、顶层高厅净空开关、导航点预算、内部无向边去重、DataAsset 内外层数组重排规范化、FName 字符串 Hash、精确 double Hash、出生楼层和 Plan 稳定排序。
