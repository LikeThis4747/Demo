# Level0 物理摆锤 V1 历史代码审阅稿（自由摆 + 中线补能）

> 状态：本审阅稿对应的 V1 已写入 `Source/Demo`、完成构建并由用户验收通过。后续“初始静止、玩家磁力蓄能并释放”的 V2 玩法尚未设计和实现，不包含在下方代码中。
>
> 对应方案：claude/plans/2026-08-05-Level0常驻物理摆锤设计方案.md。
>
> 本代码只记录 V1 的摆锤刚体、世界约束、初始释放和损耗补能；不实现玩家/追猎者受击，不接入 PCG，也不能作为 V2 磁力玩法的当前代码预览。
