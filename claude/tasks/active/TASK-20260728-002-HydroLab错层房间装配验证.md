...
## 2026-08-03 V2 中央三层塔隐藏导航坡面试验

- Owner：Codex（当前会话）。用户已明确允许开始尝试“看不见的坡面”，验收重点是不明显浮空、不出戏、不在楼梯或平台接缝卡住；不为本 Demo 新增专门上楼梯动画。
- 写入范围仍限于 `Content/Levels/Level0.umap`、本任务卡与当日计划 `DOC/DailyPlan/2026-08-03-Level0三层楼梯隐藏导航坡面验证.md`；不修改第三方 HydroLab 资产、PCG/WFC 代码、DataAsset、其他地图或追猎者代码/蓝图。
- 第一阶段只处理 `PCG_AssemblyStudy/HydroLab_ThreeFloorPCGSceneV2_Refine/T_ThreeLevelTower` 的四段楼梯。新增坡面统一归入 `NavigationTrial` 子文件夹，游戏中不可见但参与 Pawn 碰撞和 Recast；顶面目标为仅高出踏步齿顶线约 1--3cm，上下端与平台保留短重叠。
- 检查点：回读四段楼梯真实 Transform/边界 → 添加可撤回坡面 → 重建静态导航 → 检查连续 Recast → 玩家与真实追猎者双向实走 → 保存并重载 Level0。若出现明显悬空、脚底穿插、平台唇边卡顿或护栏外溢，先迭代坡面高度、宽度和端点，不扩大到其他楼梯。
- 完成门槛：绿色 NavMesh、静态碰撞和截图只作为中间证据；必须有玩家和 `BP_Pursuer` 真实走完 G0↔F3 的运行证据，并保留用户最终验收。

### 2026-08-03 隐藏坡面实际结果

- [x] 在 V2 中央三层塔的 `NavigationTrial` 子文件夹新增四段 `ZE_NavOnlyRamp` 隐藏坡面；编辑器重开后官方 MCP 回读仍为恰好 4 个 Actor，名称、标签与 Transform 均持久化，临时测试 Actor 为 0。
- [x] 四段坡面统一约为 `393.23×205×8cm`、坡度 `34.90°`，每侧收进栏杆约 `10cm`。初版视觉偏高，现已整体下压 `12cm`，最终端点顶面约比楼层/平台基准高 `4cm`，坡面主体嵌入踏步，避免明显悬空。
- [x] 渲染已完全隐藏；导航影响开启。碰撞采用能跨保存/重载稳定恢复的 `InvisibleWall` 预设，为 Pawn 提供平滑实体坡面。逐通道的“Pawn 忽略、Recast 保留”试验因重载后响应状态不稳定而撤销，没有把不稳定配置留在 Level0。
- [x] 未调整 Recast 全局代理参数；最终绿色覆盖连续通过四跑、两处 180° 转向和三个楼层落脚面。
- [x] 真实 `BP_Pursuer` 以 `1.2` 倍缩放完成 G0→F3 与 F3→G0 双向移动，终点分别约为 `(44106.75,8453.28,1008.14)` 与 `(44103.61,8596.73,109.17)`；测试后已停止 PIE 并删除临时实例。
- [x] 清洁 PIE 画面确认坡面本身不可见；只保存并重载 `/Game/Levels/Level0`，未修改第三方 HydroLab 资产、PCG/WFC 代码、DataAsset、其他地图或追猎者资产。
- [ ] 玩家键盘连续实走仍待用户亲自完成。自动化输入未形成可信移动证据，因此当前任务不标记完成；用户验收时重点检查脚底是否出戏、平台接缝是否卡顿以及双向转弯是否自然。
- WFC 交接：隐藏坡面是楼梯宏模块每一跑的内部固定子件，不是独立 1x1 Tile；后续模块数据需整体描述占地、标高、起终点、落脚区、胶囊净空、栏杆边界与隐藏行走面。

### 2026-08-03 四座双层楼梯导航配方扩展

- Owner：Codex（当前会话）。沿用本任务卡，不创建重复的 PCG 模块摆放任务。
- 用户授权范围扩展到 V2 的 `A_Stair_L1_L2`、`B_Stair_L1_L2`、`C_Stair_L2_L3`、`D_Stair_L2_L3`，仍只允许保存 Level0、截图/只读验证及任务/方案/记忆记录；不修改第三方资产、蓝图、C++、DataAsset、其他地图或 Recast 全局配置。
- [x] 盘点确认四座楼梯各有两跑原比例 `StairsB`，共需补 8 段隐藏坡面；中央塔已有 4 段不重建。
- [x] 护栏几何审计确认四个 `Guard_Upper_N_Tail` 到 `Guard_Upper_N_Corner` 的重复缺口均约为 `21.571cm`；其他空位属于楼梯入口或模块连接口，不应封闭。
- [x] 在四个模块根目录各建立 `NavigationTrial`，按中央塔配方添加两段 `ZE_NavOnlyRamp`；保存重载后总数为 12。
- [x] 用中央塔已验证参数修复四处尾栏缺口：长度轴 `ScaleY=1.087158497`，保持远端不动并把近端延伸到转角栏杆。
- [x] 复核隐藏渲染、`InvisibleWall`、导航影响、护栏连续性、Recast 覆盖与真实 `BP_Pursuer` 双向移动；玩家视觉/实走仍由用户最终验收。
- [x] 将隐藏坡面导航方法写入 `DOC/Design/PCG/SCIFI_HYDROLAB_MODULE_TABLE.md`，明确它是本项目 HydroLab 模块配方，不是 UE 官方楼梯导航概念。

### 2026-08-03 四座双层楼梯实际结果

- A/B/C/D 各新增 2 段原比例 `StairsB` 配套坡面，共 8 段；与中央塔 4 段合计 12。每段均为 Engine Cube、约 `393.23×205×8cm`、`34.90°`，带 `ZE_NavOnlyRamp` 标签。
- 保存并重载 Level0 后，12/12 坡面均保持 `bVisible=false`、`bHiddenInGame=true`、导航影响开启、反射/光追/投影关闭，并保持 `InvisibleWall + QueryAndPhysics`；A/B/C/D/T 的 `NavigationTrial` 数量分别为 `2/2/2/2/4`。
- 四个尾栏到转角栏杆的重载后间距均为约 `0.0000003cm`；楼梯入口和模块连接口未被误封。
- 真实 `BP_Pursuer` 以 `1.2` 倍缩放完成八次有效双向路线：A 终点约 `(42761.74,6049.93,560.03)` / `(42765.02,5806.93,124.66)`；B 约 `(45715.05,6041.59,546.32)` / `(45766.24,5815.72,123.86)`；C 约 `(42766.40,10235.13,1008.32)` / 远端重测 `(42982.36,10007.50,559.17)`；D 约 `(45981.29,10222.82,1008.14)` / `(45989.02,10016.00,559.17)`。
- C 的首次下行目标离楼梯口过近，追猎者在攻击距离内于坡面尾部停下；将目标移到楼梯口外侧后完整落到二层平面。该重测用于区分 AI 攻击距离提前停止与导航断路，不把首次位置算作通行结论。
- 测试结束后 PIE 已停止，临时 `ZE_TemporaryNavTest` Actor 为 0；再次保存并重载后 `/Game/Levels/Level0` 非脏，12 段坡面仍在。
- 玩家手动连续实走和脚底观感仍待用户验收，因此任务继续保持 Active。

## 2026-08-03 玩家楼梯手感精修试验

- Owner：Codex / 当前会话。用户已授权继续尝试；资产写入仍只限 `/Game/Levels/Level0`。玩家蓝图由用户按 Codex 给出的路径手动修改，Codex 不写蓝图、C++、第三方素材、其他地图或 Recast 全局配置。
- 问题证据：玩家 `BP_ZeroEscapeCharacter` 当前 `MaxWalkSpeed=450` 且 `bMaintainHorizontalGroundVelocity=true`；约 `34.90°` 的坡面会让沿坡实际速度约增至 `548.7cm/s`。相机弹簧臂未开启 Camera Lag。`SM_HydroLab_StairsB` 仍为 `BlockAll + QueryAndPhysics`，资产包含 22 个逐级凸包碰撞；隐藏坡面也阻挡 Pawn，存在双地面竞争。`InvisibleWall` 还会阻挡 Camera 通道。
