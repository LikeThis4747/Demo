 ...
 * 边界：不使用 Tick、不强制全局导航 Build、不在同一 World 自动换 Seed 重入，也不创建一次性导航探针。
 */
...
namespace
{
	...
	constexpr float StairGuidancePointLightIntensity = 16000.0f;
	constexpr float StairGuidancePointLightRadiusCm = 2000.0f;
	...
	const FZeroEscapeGenerationRequest& Request)
{
	...
	bHasAcceptedGenerationRequest = true;
	...
	ActiveCanonicalLayoutHash = 0;
	ActiveOrdinaryCellCount = 0;
	...
	ActiveCanonicalLayoutHash = CandidatePlan.CanonicalLayoutHash;
	ActiveOrdinaryCellCount = CandidatePlan.OrdinaryCells.Num();
	ActiveStructureCount = CandidatePlan.Structures.Num();
	...
	FZeroEscapeGenerationReport& InOutReport)
{
	...
	return SpawnConfiguredLights(
		...
		BuildResult.StairGuidancePointLightLocalTransforms,
		InOutReport);
}
...
bool AZeroEscapeRuntimeLevelGenerator::SpawnConfiguredLights(
	...
	const TConstArrayView<FTransform> StairGuidancePointLightLocalTransforms,
	FZeroEscapeGenerationReport& InOutReport)
{
	...
	for (const FTransform& LocalTransform : StairGuidancePointLightLocalTransforms)
	{
		FTransform WorldTransform;
		...
		if (!IsValid(PointLight) || !IsValid(PointComponent))
		{
			return FailReport(
				...
				EZeroEscapeGenerationFailure::InstantiationFailed,
				INDEX_NONE,
				TEXT("生成楼梯入口/出口引导点光源失败。"));
		}

		PointComponent->SetMobility(EComponentMobility::Movable);
		PointComponent->SetLightColor(StairGuidanceBlue);
		PointComponent->SetIntensity(StairGuidancePointLightIntensity);
		PointComponent->SetAttenuationRadius(StairGuidancePointLightRadiusCm);
	...
	const int64 CallbackOperationId)
{
	...
	FailReport(
		...
		INDEX_NONE,
		FString::Printf(
			TEXT("等待目标 RecastNavMesh 更新超过 %.2f 秒；submitted=%d observed=%d target_event=%d。"),
			ActiveNavigationTimeoutSeconds,
			bNavigationGeometrySubmitted ? 1 : 0,
			bObservedNavigationBuild ? 1 : 0,
			bReceivedTargetNavigationCompletion ? 1 : 0));
	ClearGeneratedSceneInternal();
	FinishGeneration(false, FailedReport);
}
	...
	for (const FIntVector Address : Addresses)
	{
		...
		if (FMath::Abs(ProjectedLocal.Z - ExpectedFloorZ) >= LastPlan.FloorHeightCm * 0.5)
		{
			return FailReport(
				...
				INDEX_NONE,
				FString::Printf(
					TEXT("导航代表点 (%d,%d,%d) 投射到错误楼层：local_z=%.1f expected_z=%.1f。"),
					Address.X, Address.Y, Address.Z,
					ProjectedLocal.Z, ExpectedFloorZ));
		}
	...
	for (int32 Index = 1; Index < ProjectedLocations.Num(); ++Index)
	{
		...
		if (!bPathExists)
		{
			return FailReport(
				...
				INDEX_NONE,
				FString::Printf(
					TEXT("追猎者到导航代表点 %d=(%d,%d,%d) 不存在完整路径。"),
					Index,
					Addresses[Index].X,
					Addresses[Index].Y,
					Addresses[Index].Z));
	...
	const FZeroEscapeGenerationReport& Report)
{
	...
	UE_LOG(
		...
		*GetStableEnumName(LastReport.Failure),
		...
		static_cast<long long>(ActiveCanonicalLayoutHash),
		LastReport.Metrics.TotalMilliseconds,
