...
namespace ZeroEscape::LevelGeneration
{
	namespace GenerationCorePrivate
	{
			...
			const int32 FloorCount)
		{
			...
			FIntVector Previous = FIntVector::ZeroValue;
		...
		FZeroEscapeGenerationReport& OutReport)
	{
		...
		OutInput.StructureDefinitions.Sort(
			[](const FZeroEscapeStructureDefinition& A, const FZeroEscapeStructureDefinition& B)
			{
				...
				return GenerationCorePrivate::NameLess(A.DefinitionId, B.DefinitionId);
			});
		...
		const FZeroEscapeGeneratedLevelPlan& Plan)
	{
		...
		for (const FZeroEscapeGeneratedStructure& Structure : Plan.Structures)
		{
			...
			TSet<FIntVector> StructureWalkableCells;
			for (const FIntVector Coordinate : Structure.WalkableCells)
			{
				StructureWalkableCells.Add(Coordinate);
			}
			HashUInt64(Hash, Structure.InternalConnections.Num());
			FZeroEscapeGeneratedCellConnection PreviousConnection;
			bool bHasPreviousConnection = false;
				...
				Structure.InternalConnections)
			{
				if (!IsInsideBuilding(Connection.FirstCoordinate, Plan.GridSize, Plan.FloorCount)
					...
					|| !StructureWalkableCells.Contains(Connection.FirstCoordinate)
					|| !StructureWalkableCells.Contains(Connection.SecondCoordinate)
					|| !GenerationCorePrivate::CoordinateLess(
						Connection.FirstCoordinate, Connection.SecondCoordinate)
					|| (bHasPreviousConnection
						&& !GenerationCorePrivate::ConnectionLess(
				...
				PreviousConnection = Connection;
				bHasPreviousConnection = true;
			}

			HashUInt64(Hash, Structure.Openings.Num());
			FName PreviousOpeningId = NAME_None;
			bool bHasPreviousOpening = false;
			for (const FZeroEscapeGeneratedStructureOpening& Opening : Structure.Openings)
			{
				if (Opening.OpeningId.IsNone()
					...
					|| !StructureWalkableCells.Contains(Opening.StructureCoordinate)
					|| (bHasPreviousOpening
						&& !GenerationCorePrivate::NameLess(
							PreviousOpeningId, Opening.OpeningId)))
				{
					return 0;
				}
				...
				PreviousOpeningId = Opening.OpeningId;
				bHasPreviousOpening = true;
			}

			HashUInt64(Hash, Structure.Landings.Num());
			FName PreviousLandingId = NAME_None;
			bool bHasPreviousLanding = false;
			for (const FZeroEscapeGeneratedStructureLanding& Landing : Structure.Landings)
			{
				if (Landing.LandingId.IsNone()
					...
					|| !StructureWalkableCells.Contains(Landing.Coordinate)
					|| (bHasPreviousLanding
						&& !GenerationCorePrivate::NameLess(
							PreviousLandingId, Landing.LandingId)))
				{
					return 0;
				}
	...
	bool FGenerationCore::IsFinitePositiveScaleTransform(const FTransform& Transform)
	{
