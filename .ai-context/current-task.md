# Current Task

- ID：`TASK-20260727-005`
- 目标：按用户授权，为运行时 PCG 场景加入约每两个有效逻辑格一盏的 HydroLab 原始顶灯，并先完成源码、构建和 Presentation DataAsset 装配。
- 当前范围：只修改四个现有 PCG C++ 文件、项目自有 `DA_Presentation_SciFiHydroLab` 及对应任务/计划文档；不修改 WFC、Level0、第三方灯 Blueprint、测试 Harness 或小地图。
- 已确认规则：按 `(GridX+GridY)%2` 选择较少奇偶组，平局选 Start 组；若较少组不含 Start，只额外补 Start；灯位为逻辑格中心、`CeilingPivotZCm`、LampA `Roll=180°`。
- 当前阶段：实现前快照 `988b518` 已推送内部工蜂；四个 C++ 文件已完成首轮最小修改并通过 `git diff --check`，等待用户关闭 UE 后执行 UE5.8 构建。
- 验收：构建成功、DataAsset 配置保存；用户在正常主视口 PIE 检查 Seed 12345 与至少一个不同 Seed，确认灯量/方向/亮度/无残留；未经验收不标记完成。
- 后续：顶灯验收后再单独讨论小地图最小方案。
